...
export default function MobileHome() {
  ...
  return (
    <div className={`${poppins.variable} ${syne.variable} bg-[#fbfdff] text-[#0C0A0E] font-body overflow-x-hidden w-full min-h-screen`}>
      ...
      {/* NAVBAR */}
      <nav className={`fixed top-0 left-0 w-full z-40 px-6 py-4 transition-all duration-300 ${!isMenuOpen ? "bg-[#fbfdff]/90 backdrop-blur-md border-b border-black/5" : "bg-transparent"}`}>
        <div className="flex justify-between items-center">
          <div className={`flex items-center gap-3 transition-opacity duration-300 ${isMenuOpen || showTeamModal ? "opacity-0" : "opacity-100"}`}>
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="relative z-50 w-10 h-10 flex flex-col justify-center items-end gap-1.5 focus:outline-none">
            <span className={`block h-0.5 bg-[#001439] transition-all duration-300 ${isMenuOpen ? "w-6 -rotate-45 translate-y-2 bg-white" : "w-6"}`} />
            <span className={`block h-0.5 bg-[#001439] transition-all duration-300 ${isMenuOpen ? "w-6 opacity-0" : "w-4"}`} />
            <span className={`block h-0.5 bg-[#001439] transition-all duration-300 ${isMenuOpen ? "w-6 rotate-45 -translate-y-2 bg-white" : "w-3"}`} />
          </button>
        </div>
      </nav>

      {/* MENU */}
      <div className={`fixed inset-0 z-30 bg-[#001439] flex flex-col items-center justify-center transition-all duration-500 ${isMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
         <div className="flex flex-col gap-6 text-center">
            {['home', 'vision', 'society', 'team', 'contact'].map((item, i) => (
                <button key={item} onClick={() => scrollTo(item)} className="text-3xl font-bold text-white uppercase tracking-widest" style={{ fontFamily: "var(--font-syne)" }}>{item}</button>
            ))}
            <Link href="/events" className="text-xl font-medium text-white/50 uppercase tracking-widest mt-4">Events</Link>
         </div>
      </div>
      ...
      <main className={`transition-opacity duration-1000 flex flex-col ${isLoading ? "opacity-0" : "opacity-100"}`}>
        ...
        <div className="-mt-10 relative z-10">
           <MobileVision id="vision" />
        </div>
        
        <div className="-mt-12 relative z-20">
            <MobileSociety id="society" />
        </div>
        
        <div className="-mt-12 relative z-30">
            <MobileTeam id="team" onOpenModal={() => setShowTeamModal(true)} />
        </div>
  ...
  return (
    <section id={id} className="relative min-h-[105vh] flex flex-col justify-center px-6 pt-32 pb-40 overflow-hidden bg-[#fbfdff]">
      ...
      <div className="relative z-10 space-y-6">
        <Reveal delay={100}>
            <p className="text-[#3B82C4] font-bold tracking-[0.25em] text-xs uppercase">Welcome to</p>
        </Reveal>
        
        <Reveal delay={200}>
            <h1 className="text-[19vw] leading-[0.85] font-black text-[#001439] tracking-tighter" style={{ fontFamily: "var(--font-syne)" }}>IEEE</h1>
        </Reveal>
        
        <Reveal delay={300}>
            <h2 className="text-3xl font-bold text-[#001439]/90" style={{ fontFamily: "var(--font-syne)" }}>Faculty of<br />Technology</h2>
        </Reveal>
        
        <Reveal delay={400}>
            <p className="text-[#001439]/70 text-lg leading-relaxed max-w-[85%] pt-2">Innovating for tomorrow, building systems that scale, and fostering a community of engineers.</p>
        </Reveal>
        
        <Reveal delay={500}>
            <div className="pt-8 flex gap-4">
                <button onClick={() => document.getElementById('vision').scrollIntoView({ behavior: 'smooth' })} className="px-8 py-4 bg-[#001439] text-white rounded-full font-bold uppercase tracking-wider text-xs shadow-xl active:scale-95 transition-transform">
                    Start Exploring
                </button>
            </div>
        </Reveal>
...
function MobileVision({ id }) {
  ...
  return (
    <section id={id} className="relative px-6 pt-16 pb-32 bg-white rounded-t-[3rem] shadow-[0_-25px_50px_rgba(0,0,0,0.05)] overflow-hidden">
      ...
      <div className="absolute top-12 right-[-10%] pointer-events-none opacity-[0.04] z-0">
         <h2 className="text-[12rem] font-black text-[#001439] leading-none" style={{ fontFamily: "var(--font-syne)" }}>VISION</h2>
      </div>

      <div className="relative z-10">
        <Reveal>
            <div className="mb-12">
                <p className="text-xs font-bold tracking-widest text-blue-400 uppercase mb-2">Core Values</p>
                <h2 className="text-4xl font-black text-[#001439]" style={{ fontFamily: "var(--font-syne)" }}>Our Vision</h2>
            </div>
        </Reveal>
...
function MobileSociety({ id }) {
  ...
  const toggleCard = (index) => {
    if (openCardIndex === index) {
        setOpenCardIndex(null); // Close if clicking same
    } else {
        setOpenCardIndex(index); // Open new one
    }
  };
  ...
  const societies = [
    { 
        ...
        color: "#3B82C4", 
        bg: "#EBF4FD", 
        desc: "Advancing theory, practice, and application of computer and information processing science.",
        roles: [
            { title: "Chair", name: "Alex Richardson" },
            { title: "Vice Chair", name: "Sarah Chen" },
            { title: "Tech Lead", name: "David Kim" },
            { title: "Outreach", name: "Emma Watson" }
        ]
    },
    { 
        title: "Power & Energy", 
        ...
        color: "#5B9BD5", 
        bg: "#eef7ff", 
        desc: "Leading the future of power and energy to foster technological innovation.",
        roles: [
            { title: "Chair", name: "Alex Richardson" },
            { title: "Vice Chair", name: "Sarah Chen" },
            { title: "Tech Lead", name: "David Kim" },
    ...
    { 
        ...
        color: "#6B7280", 
        bg: "#F3F4F6", 
        desc: "Fostering the development of robotics and automation for the benefit of humanity.",
        roles: [
            { title: "Chair", name: "Alex Richardson" },
            { title: "Vice Chair", name: "Sarah Chen" },
            { title: "Tech Lead", name: "David Kim" },
    ...
    <section id={id} className="pt-24 pb-32 bg-[#f8fafc] rounded-t-[3rem] shadow-[0_-25px_50px_rgba(0,0,0,0.05)] relative overflow-hidden">
      <Reveal>
        <div className="px-6 mb-10 relative z-10">
            <p className="text-xs font-bold tracking-widest text-gray-400 uppercase mb-2">Communities</p>
            <h2 className="text-4xl font-black text-[#001439]" style={{ fontFamily: "var(--font-syne)" }}>Societies</h2>
        </div>
      </Reveal>
          ...
          return (
            <Reveal key={i} delay={i * 100}>
                <div 
                    ...
                    className="w-full bg-white rounded-[2rem] p-8 border border-black/5 shadow-sm active:scale-[0.98] transition-transform duration-300 cursor-pointer"
                >
                    {/* Header */}
                    <div className="flex justify-between items-start mb-6">
                        <div className="w-14 h-14"><SocietyIcon type={s.icon} color={s.color} /></div>
                        {/* Chevron Animation */}
                        <div className={`p-2 rounded-full bg-black/5 text-black/60 transition-transform duration-500 ${isOpen ? "rotate-180 bg-black/10" : ""}`}>
                        ...
                        <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                            {s.roles.map((role, idx) => (
                                <div key={idx} className="flex flex-col">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">{role.title}</span>
                                    <span className="text-sm font-semibold text-[#001439]" style={{ fontFamily: "var(--font-syne)" }}>{role.name}</span>
                                </div>
                            ))}
                        </div>
                        <div className="mt-6 text-xs font-bold text-[#001439]/30 uppercase tracking-widest text-center">Tap to collapse</div>
                    </div>

                    <div className={`w-full h-1 rounded-full bg-gray-100 overflow-hidden mt-6 transition-all duration-500 ${isOpen ? "opacity-0" : "opacity-100"}`}>
                        <div className="h-full w-1/3 rounded-full" style={{ backgroundColor: s.color }}></div>
                    </div>
                </div>
            </Reveal>
          );
    ...
    <section id={id} className="pt-24 pb-32 bg-[#0a0a0a] text-white rounded-t-[3rem] shadow-[0_-25px_50px_rgba(0,0,0,0.2)] relative z-10">
      <Reveal>
        <div className="px-6 mb-10">
            <h2 className="text-4xl font-black" style={{ fontFamily: "var(--font-syne)" }}>The Team</h2>
            <p className="text-white/50 text-sm mt-2">The minds behind the magic.</p>
        </div>
      </Reveal>
        ...
        {randomPreview.map((t, i) => (
          <Reveal key={i} delay={i * 50}>
            <div className="bg-white/5 rounded-2xl p-4 flex flex-col items-center gap-3 border border-white/5">
                <div className="w-20 h-20 rounded-full p-0.5 relative overflow-hidden bg-gradient-to-b from-white/20 to-transparent">
                    <div className="w-full h-full rounded-full relative overflow-hidden bg-gray-800">
                        <img src={t.img} alt={t.name} className="object-cover w-full h-full" />
                    </div>
                </div>
                <div className="text-center">
                    <p className="text-sm font-bold text-white leading-tight">{t.name.split(" ")[0]}</p>
      ...
      <Reveal delay={200}>
        <div className="px-6">
            <button onClick={onOpenModal} className="w-full py-4 rounded-xl bg-[#3B82C4] text-white font-bold uppercase tracking-widest text-xs shadow-lg active:scale-95 transition-transform">
                Meet Everyone
            </button>
        </div>
      </Reveal>
...
function MobileContact({ id }) {
  return (
    <section id={id} className="px-6 pt-24 pb-12 bg-[#fbfdff] rounded-t-[3rem] shadow-[0_-25px_50px_rgba(0,0,0,0.1)] relative z-20">
       <Reveal>
        <h2 className="text-5xl font-black text-[#001439] mb-8 leading-[0.9]" style={{ fontFamily: "var(--font-syne)" }}>
            Let's <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3B82C4] to-blue-600">Build.</span>
        </h2>
       </Reveal>

       <Reveal delay={200}>
        <div className="bg-white rounded-[2rem] p-8 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.1)] border border-blue-50/50 space-y-8">
            <div>
                <p className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-2 flex items-center gap-2">Visit</p>
                <p className="text-[#001439] font-medium text-lg">Faculty of Technology<br/>University of Delhi</p>
            </div>
            <div className="w-full h-px bg-gray-100" />
            <div>
                <p className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-2 flex items-center gap-2">Email</p>
                <a href="mailto:ieee@fot.du.ac.in" className="text-lg font-bold text-[#001439] break-all">ieee@fot.du.ac.in</a>
            </div>
            <div className="flex gap-3 pt-2">
                <SocialBtn icon={<LinkedinIcon />} />
                <SocialBtn icon={<InstagramIcon />} />
                <SocialBtn icon={<MailIcon />} />
            </div>
        </div>
       </Reveal>
...
function TeamGridModal({ isOpen, onClose }) {
    return (
        <div className={`fixed inset-0 z-[60] bg-[#001439] transition-all duration-500 flex flex-col ${isOpen ? "opacity-100 pointer-events-auto translate-y-0" : "opacity-0 pointer-events-none translate-y-10"}`}>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 pb-20">
                <div className="grid grid-cols-2 gap-4">
                    {ALL_MEMBERS.map((t, i) => (
                        <div key={i} className="bg-white/5 rounded-2xl p-4 flex flex-col items-center gap-3 border border-white/5">
                            <div className="w-20 h-20 rounded-full overflow-hidden bg-gray-800 border-2 border-white/10">
                                <img src={t.img} alt={t.name} className="w-full h-full object-cover" />
                            </div>
                            <div className="text-center">
                                <p className="text-white text-sm font-bold leading-tight">{t.name}</p>
                                <p className="text-white/50 text-[10px] uppercase tracking-wider mt-1">{t.role}</p>
                            </div>
                        </div>
                    ))}
                </div>
...
function SocialBtn({ icon }) {
    return <button className="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center text-[#001439] hover:bg-blue-50 hover:scale-105 active:scale-95 transition-all border border-black/5">{icon}</button>
}
